...
-- ============================================
-- ANALYTICS CLEARING FUNCTION (Manual Use)
-- ============================================
-- Function to clear all analytics data from page_views and link_clicks tables
-- This function can be called manually via API or external cron jobs

CREATE OR REPLACE FUNCTION clear_analytics_data()
RETURNS TABLE (
    page_views_deleted BIGINT,
    link_clicks_deleted BIGINT,
    cleared_at TIMESTAMPTZ
) AS $$
DECLARE
    v_page_views_count BIGINT;
    v_link_clicks_count BIGINT;
BEGIN
    -- Get count of records before deletion (for reporting)
    SELECT COUNT(*) INTO v_page_views_count FROM page_views;
    SELECT COUNT(*) INTO v_link_clicks_count FROM link_clicks;
    
    -- Delete all records from page_views table
    DELETE FROM page_views;
    
    -- Delete all records from link_clicks table
    DELETE FROM link_clicks;
    
    -- Reset denormalized counters in links table
    UPDATE links
    SET click_count = 0;
...
-- Grant execute permission on the function (for local PostgreSQL)
-- All functions are accessible to the database user (postgres)
-- Application layer handles access control
-- GRANT EXECUTE ON FUNCTION clear_analytics_data() TO PUBLIC;

COMMENT ON FUNCTION clear_analytics_data() IS 
'Clears all analytics data from page_views and link_clicks tables, and resets denormalized counters. Can be called manually or via external cron jobs.';
